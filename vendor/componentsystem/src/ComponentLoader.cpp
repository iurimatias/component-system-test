...
auto Nedrysoft::ComponentSystem::ComponentLoader::addComponents(const QString &componentFolder) -> void {
    ...
    if (!applicationDebugBuild) {
        qWarning() << "Application was built with QT_DEBUG but has loaded RELEASE qt libraries, component system will load DEBUG components instead.";

        applicationDebugBuild = true;
    }
#else
    if (applicationDebugBuild) {
        qWarning() << "Application was built with QT_NO_DEBUG but has loaded DEBUG qt libraries, component system will load RELEASE components instead.";

        applicationDebugBuild = false;
    }
#endif
#endif
    qInfo() << "Searching folder for components" << componentFolder;
    ...
    while (dir.hasNext()) {
        ...
        qInfo() << "Found Component" << componentFilename;
...
auto Nedrysoft::ComponentSystem::ComponentLoader::loadComponents(
    ...
    for (auto component : resolvedLoadList) {
        if (component->m_loadFlags) {
            qWarning() << "component" << component->name() << "was not loaded." << loadFlagString(component->m_loadFlags);
        ...
        if (!pluginLoader->load()) {
            ...
            qWarning() << "component" << component->name() << "was not loaded." << loadFlagString(component->m_loadFlags) << "[" << pluginLoader->errorString() << "]";
